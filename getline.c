...
ssize_t get_input(info_t *info)
{
	...
	if (len) /* we have commands left in the chain buffer */
	{
		...
		if (i >= len) /* have we reached end of buffer? */
		{
	...
	*buf_p = buf; /* else not a chain, pass buffer from _getline() */
	return (s); /* return len of buffer from _getline() */
}
...
/**
 * _getline - gets nxt line of input from STDIN
 * @info: struct of the parameter
 * @ptr: address of pointer to buffer, preallocated or NULL
 * @length: length of preallocated ptr buffer if not NULL
 *
 * Return: s
 */
